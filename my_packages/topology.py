...
def refMN_generate(SPEC_THRESHOLD, PEAK_THRESHOLD, SPEC_SIM_MATRIX, IDXtoCCMSID, ALGORITHM, GNPS_INFO):
    ...
    if ALGORITHM == 'neutral_loss':
        N_PEAK_MATRIX = np.load(f'../msdb/data/matrix/neutral_loss_peak_matrix.npy')

    elif ALGORITHM == 'modified_cosine':
        N_PEAK_MATRIX = np.load(f'../msdb/data/matrix/neutral_loss_peak_matrix.npy')

    else:
        N_PEAK_MATRIX = np.load(f'../msdb/data/matrix/cosine_peak_matrix.npy')
    ...
    MN_output = f'../msdb/data/network/MN_{ALGORITHM}_{SPEC_THRESHOLD}_{PEAK_THRESHOLD}.graphml'
...
def get_groups():
    ...
    group4 = ['penrose_size','inner_product','canberra','clark','hellinger','divergence', 'fidelity', 'bhattacharya_1', 'improved_similarity', 'matusita', 'mean_character', 'spectral_contrast_angle', 'squared_chord', 'wave_hedges', 'weighted_dot_product', 'whittaker_index_of_association']
